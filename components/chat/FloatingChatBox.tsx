...
export const FloatingChatBox: React.FC<FloatingChatBoxProps> = ({ className }) => {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `${SOCKET_URL}/chat/conversations/${activeConversation.id}/avatar`,
        {
          method: 'POST',
          headers: {
      ...
      if (avatarImg) {
        avatarImg.src = `${SOCKET_URL}/chat/conversations/${activeConversation.id}/avatar?t=${timestamp}`;
      }
    } catch (error) {
                ...
                <>
                  <Avatar className="h-8 w-8">
                    {activeConversation.type === 'group' ? (
                      <>
                        <AvatarImage
                          src={`${SOCKET_URL}/chat/conversations/${activeConversation.id}/avatar`}
                      ...
                      <>
                        {getConversationAvatar(activeConversation) && (
                          <AvatarImage
                            src={`${SOCKET_URL}${getConversationAvatar(activeConversation)}`}
                          />
                        )}
                        <AvatarFallback>
                        ...
                        return (
                          ...
                          >
                            <div className="relative">
                              <Avatar className="h-8 w-8">
                                <AvatarImage
                                  src={`${SOCKET_URL}/users/${participant.userId}/avatar`}
                              ...
                              <span
                                className={`absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-primary ${isOnline ? 'bg-green-500' : 'bg-gray-400'
                                  }`}
                              />
                            </div>
