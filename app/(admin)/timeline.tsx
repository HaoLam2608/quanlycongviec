...
export default function AdminTimelinePage() {
  ...
  const loadProjects = async () => {
    try {
      setLoading(true);
      const projsData = await fetchProjects();
      const projs = Array.isArray(projsData) ? projsData : (projsData.projects || projsData.data || []);
      setProjects(projs || []);
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', padding: 16 }}>
          <View style={{ backgroundColor: '#fff', borderRadius: 12, maxHeight: '80%', padding: 16 }}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity 
                  onPress={() => { setSelectedProject(item); setShowProjectPicker(false); setProjectSearch(''); }} 
                  style={{ 
                    paddingVertical: 14, 
                    paddingHorizontal: 12,
                    borderBottomWidth: 1, 
