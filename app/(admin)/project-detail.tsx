...
export default function ProjectDetail() {
    ...
    const loadDocuments = async () => {
        try {
            ...
            const transformedDocs = docs.map((doc: any) => ({
                ...
                kichThuoc: doc.size || doc.kichThuoc || 0,
    ...
    const handleDeleteDocument = async (docId: number, tenTaiLieu?: string) => {
        Alert.alert(
            'Xác nhận xóa',
            `Bạn có chắc chắn muốn xóa tài liệu "${tenTaiLieu || ''}"?`,
            [
                { text: 'Hủy', style: 'cancel' },
                {
                    text: 'Xóa',
                    ...
                    onPress: async () => {
                        try {
                            ...
                            const errorMsg = error?.response?.data?.message || 'Không thể xóa tài liệu';
                            Alert.alert('Lỗi', errorMsg);
                        }
                    }
                }
    ...
    const getFileColor = (mimeType: string): string => {
        if (mimeType.includes('pdf')) return '#FF0000';
        ...
        if (mimeType.includes('powerpoint') || mimeType.includes('presentation')) return '#FF5722';
        return '#757575';
    };
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <TextInput
                            ...
                            style={[styles.input, { height: 80, marginTop: 16 }]}
                            multiline
                        />
