        ...
        <div className="min-h-screen bg-gray-50 p-3 sm:p-6">
            <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8">
                ...
                <div className="space-y-4">
                    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
                            <div className="text-left sm:text-right px-3 py-2 bg-white rounded-lg shadow-sm border border-gray-200">
                                <p className="text-xs sm:text-sm text-gray-500">Tổng số chờ phê duyệt</p>
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2 disabled:opacity-50 w-full sm:w-auto"
                            >
                            ...
                            {totalCount === 0 ? (
                                ...
                                <>
                                    ...
                                    {(filter === 'all' || filter === 'tasks') && filteredTasks.map(task => (
                                        <div key={`task-${task.id}`} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors">
                                    ...
                                    {(filter === 'all' || filter === 'subtasks') && filteredSubtasks.map(subtask => (
                                        <div key={`subtask-${subtask.id}`} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors">
                                            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3 mb-2">
                            ...
                            {(claimRequests.filter(r => r.status === 'pending').length + joinRequests.filter(r => !r.meta.assignmentId).length) === 0 ? (
                                ...
                                <>
                                    ...
                                    {(requestFilter === 'all' || requestFilter === 'claim') && claimRequests.filter(r => r.status === 'pending').map(request => (
                                        <div key={`claim-${request.id}`} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors">
                                            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3 mb-2">
                                    ...
                                    {(requestFilter === 'all' || requestFilter === 'join') && joinRequests.filter(r => !r.meta.assignmentId).map(request => {
                                        ...
                                        return (
                                            <div
                                                key={`join-${request.id}`}
                                                className={`p-4 sm:p-6 transition-colors ${isProcessed ? 'bg-gray-50 opacity-70' : 'hover:bg-gray-50'}`}
                                            >
                                                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                                                    <div className="flex-1">
                ...
                {isModalOpen && selectedItem && (
                    <div className="fixed inset-0 backdrop-blur-[0px] flex items-center justify-center z-50 p-4 sm:p-6">
