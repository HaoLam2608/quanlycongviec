...
module.exports = {
    async up(queryInterface, Sequelize) {
        // Lấy id user theo manv/email
        const [users] = await queryInterface.sequelize.query("SELECT id, manv, email FROM Users");
        const getId = (manv) => users.find(u => u.manv === manv)?.id;
        await queryInterface.bulkInsert('DuAns', [
            // Dự án đã hoàn thành
            {
                ...
                userId: getId('QLY001'), // Trần Thành Đạt (Manager)
                createdAt: new Date(),
                updatedAt: new Date()
            },
            {
                ...
                status: 'da_hoan_thanh',
                userId: getId('QLY002'), // Lê Thị Hoài Thu (Manager)
                createdAt: new Date(),
                updatedAt: new Date()
            },
