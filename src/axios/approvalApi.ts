...
const getMyJoinRequests = async () => {
    try {
        ...
        // Always return empty list to prevent app crash
        // Backend has column error that needs to be fixed
        console.warn('Error loading join requests:', err.response?.data?.error || err.message);
